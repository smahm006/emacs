...
#+begin_src emacs-lisp
  (use-package yaml
    ...
    (defun smahm006/yaml-format ()
  	...
  	(interactive)
  	(let ((output (shell-command-to-string (format "yamlfmt -quiet %s" buffer-file-name))))
  	  (message "%s" (string-trim output)))
  	(smahm006/revert-buffer-no-confirm))
    (defun smahm006/yaml-check ()
      "Compile current buffer file with yaml."
      (interactive)
      (compile (format "yamllint -f standard %s" buffer-file-name)))
    :config
...
#+begin_src emacs-lisp
  (use-package python
    ...
    (defun smahm006/python-format ()
  	...
  	(interactive)
  	(let ((output (shell-command-to-string (format "ruff format %s" buffer-file-name))))
  	  (message "%s" (string-trim output)))
  	(smahm006/revert-buffer-no-confirm))
    (defun smahm006/python-fix ()
  	"Check for and fix any errors"
  	(interactive)
  	(let ((output (shell-command-to-string (format "ruff check --fix %s" buffer-file-name))))
  	  (message "%s" (string-trim output)))
  	(smahm006/revert-buffer-no-confirm))
...
#+begin_src emacs-lisp
  (use-package go
    ...
    (defun smahm006/go-format ()
      ...
      (interactive)
    	(let ((output (shell-command-to-string (format "goimports -w %s && gofumpt -w %s" buffer-file-name buffer-file-name))))
    	  (message "%s" (string-trim output)))
    	(smahm006/revert-buffer-no-confirm))
