...
#+begin_src emacs-lisp
  ...
  Additionally installed libraries from requirements.txt if it exists."
    ...
    (defun smahm/python-fix ()
      "Compile current buffer file with python."
      (interactive)
      (compile (format "ruff check --fix %s" buffer-file-name)))
...
#+begin_src emacs-lisp
  (use-package c
    ...
    (defun smahm/c-c++-format ()
      "Compile current buffer file with c."
      (interactive)
      (compile (format "clang-format -i %s" buffer-file-name)))
    (defun smahm/c-c++-check ()
      "Compile current buffer file with c."
      (interactive)
...
#+begin_src emacs-lisp
  (pretty-hydra-define hydra-c
    (:hint nil :forein-keys warn :quit-key "C-g" :title (with-mdicon "nf-md-language_c" "" 1 -0.05))
    ("Run"
     (("r" smahm/c-run "run"))
     "Format"
     (("f" smahm/c-c++-format "format")
  	("c" smahm/c-c++-check "check"))))
  (pretty-hydra-define hydra-c++
    (:hint nil :forein-keys warn :quit-key "C-g" :title (with-mdicon "nf-md-language_c" "++" 1 -0.05))
    ("Run"
     (("r" smahm/c++-run "run"))
